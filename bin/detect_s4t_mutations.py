...
import click
import pairtools
import pairtools.lib.headerops
import pyximport
...
def detect_s4t_mutations(pairsam_path, output, chunksize, drop_sam):
    """Add columns with the number of s4T-induced mutations.
    """
    fill_n_s4t_pairs(pairsam_path, output, chunksize=chunksize, drop_sam=drop_sam)


def fill_n_s4t_pairs(in_path, out_path, chunksize=int(1e6), drop_sam=False):
    header, pairs_body = pairtools.lib.headerops.get_header(
        pairtools.lib.fileio.auto_open(in_path, "r")
    )

    cols = pairtools.lib.headerops.extract_column_names(header)
    ...
    new_header = pairtools.lib.headerops.extract_fields(header, "columns", save_rest=True)[
        1
    ]
    new_header.append("#columns: " + " ".join(new_cols))

    samheader_lines = pairtools.lib.headerops.extract_fields(header, "samheader")
    samheader = pysam.AlignmentHeader.from_text("\n".join(samheader_lines))

    out_stream = (
        pairtools.lib.fileio.auto_open(out_path, nproc=3, mode="w")
        if out_path
        else sys.stdout
    )
    ...
    for chunk_df in chunk_iterator:
        ...
        if drop_sam:
            chunk_df.drop(columns=["sam1", "sam2"], inplace=True, axis=1)
...
def get_mismatches_samtxt(sam_col, samheader):
    sam_strs = [
        sam_str.replace(pairtools.lib.pairsam_format.SAM_SEP, "\t")
        for sam_str in sam_col.split(pairtools.lib.pairsam_format.INTER_SAM_SEP)
    ]
